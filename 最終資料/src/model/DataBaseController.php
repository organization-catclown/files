...
class DataBaseController
{
    ...
    public static function searchLeaveData($leaveDataClass)
    {
        ...
        try {
            ...
            $sql = "SELECT 1 as HIT FROM DUAL WHERE EXISTS (SELECT * FROM RoomManagements WHERE EndFlag = FALSE AND StudentNumber = :studentNumber)";
     ...
     * @return boolean 存在していればTRUE、していなければFALSE
     */
    public function searchTeacherCode($teacherCode)
    {
        ...
        try {
            $pdo = new PDO(DSN, USER, PASS);
            $sql = "SELECT 1 as HIT FROM DUAL WHERE EXISTS (SELECT * FROM Teachers WHERE TeacherCode = :teacherCode)";
            $stmt = $pdo->prepare($sql);
    ...
    /**
     * 教室利用管理表に未退室の生徒が残っているか確認する
     * @return boolean 未退室の生徒がいればTRUE、いなければFALSE
     */
    public function searchRoomManagements()
    {
        $pdo = null;
        $stmt = null;
        try {
            $pdo = new PDO(DSN, USER, PASS);
            $sql = "SELECT 1 as HIT FROM DUAL WHERE EXISTS (SELECT * FROM RoomManagements WHERE EndFlag = FALSE)";
            $stmt = $pdo->prepare($sql);
    ...
    /**
     * 教員表に登録されてある教員全員のメールアドレスにメールを送信する
     */
    public static function sendMail()
    {
        ...
        $to = "h1201715022@hiratagakuen.onmicrosoft.com";
        $subject = "てすとめーる";
        $message = "これは、てすとめーるです。";
        $headers = "From: h1201715021@hiratagakuen.onmicrosoft.com\n";

        $pdo = null;
        $stmt = null;
        try {
            $pdo = new PDO(DSN, USER, PASS);
            $stmt = $pdo->query("SELECT Email FROM Teachers");
            foreach ($stmt as $row) {
